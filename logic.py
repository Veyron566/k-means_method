...
def read(namef,list_of_objects):      #reading the data file
    ...
    for line in _file:
        ...
        if words[0] == '':
            continue
                         #first line contains names of variables
...
def get_list_of_variables(namef):
    _file  =  open(namef, 'r')
    list_of_variables = list()
    for line in _file:
        list_a = line.split(',')
        list_of_variables = list_a[1:]
        list_of_variables[-1] = list_of_variables[-1][0:-1]#cutting '\n'
        break
    return list_of_variables

def get_values(namef,index__):
    _file  =  open(namef, 'r')
    list_of_variables = list()
    for line in _file:
        list_a = line.split(',')
        if list_a[0] == '':
            ...
            list_of_variables.append(float(list_a[index__]))
    return list_of_variables

def get_list_of_names(namef):
    _file = open(namef,'r')
    ...
    for line in _file:
        ...
        if list_a[0] == '':
            ...
            list_of_objects.append(list_a[0])
...
def calculating(datafile,count_of_classes = 2):
    list_of_objects = list()#structure: [[name,item...item],[name,item...item]]
    read(datafile,list_of_objects)
    if count_of_classes  >  len(list_of_objects):
        return []
